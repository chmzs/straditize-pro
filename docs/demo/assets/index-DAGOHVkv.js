...
`}openProjectFile(e){let t=e.name.toLowerCase().endsWith(`.tar`)||e.name.toLowerCase().endsWith(`.tar.gz`)||e.type.includes(`tar`),n=(e,t)=>{let n=e.calibration||e.depth_calibration,r=e.columns;if(!r||!Array.isArray(r)||!n)return null;let i={dataXMin:n.dataXMin??e.roi?.x??0,dataXMax:n.dataXMax??(e.roi?e.roi.x+e.roi.w:1e3),dataYMin:n.dataYMin??n.top_px??e.roi?.y??0,dataYMax:n.dataYMax??n.bottom_px??(e.roi?e.roi.y+e.roi.h:1e3),depthTopValue:n.depthTopValue??n.top_cm??0,depthBottomValue:n.depthBottomValue??n.bottom_cm??150,unit:n.unit||`cm`,depthInterval:n.depthInterval??n.depth_interval??2,depthGridEnabled:n.depthGridEnabled??n.depth_grid_enabled??!0,isCalibrated:n.isCalibrated??n.is_calibrated??!0,top_px:n.top_px??n.dataYMin,bottom_px:n.bottom_px??n.dataYMax,top_cm:n.top_cm??n.depthTopValue,bottom_cm:n.bottom_cm??n.depthBottomValue},a=r.map((e,t)=>{let n=e.name||e.species||`Col ${t+1}`,r=e.startX??0,i=e.tickEndX??e.scaleCalib?.calibX??e.endX??r+50,a=e.endX??i,o=e.startValue??e.scaleCalib?.originVal??0,s=e.tickValue??e.scaleCalib?.calibVal??e.maxPercent??100,c=(e.controlPoints||e.points||[]).map((e,n)=>({id:e.id||`pt_${t}_${n}`,x:e.x,y:e.y,value:e.value,kind:e.kind||(e.type===`manual`||e.isManual?`manual`:`peak`),valid_segment:e.valid_segment??!0,isManual:e.kind===`manual`||e.isManual||!1,type:e.kind||e.type||`peak`,createdAt:e.createdAt||Date.now()}));return{id:e.id||`col_${t}`,name:n,species:n,color:e.color||`#38bdf8`,startX:r,endX:a,maxPercent:s,tickEndX:i,unit:e.unit||`%`,isLocked:e.isLocked||!1,curveType:e.curveType||`linear`,visible:e.visible!==!1,scale_type:e.scale_type||`linear`,startValue:o,tickValue:s,controlPoints:c,points:c,scaleCalib:e.scaleCalib||{originX:r,originVal:o,calibX:i,calibVal:s,unit:e.unit||`%`}}});return{imageSrc:t||e.image?.src||e.imageSrc||this.data.imageSrc,imageWidth:e.image?.width||e.imageWidth||this.data.imageWidth,imageHeight:e.image?.height||e.imageHeight||this.data.imageHeight,calibration:i,columns:a,activeTaxaId:e.activeTaxaId||a[0]?.id||``,selectedEntity:null}};if(t){let t=new FileReader;t.onload=e=>{try{let t=e.target?.result;if(!t)return;let r=v.extract(t);if(r.length===0){alert(`读取 .tar 归档失败：未找到有效的文件块。`);return}let i=null,a=null;for(let e of r)if(e.name.endsWith(`straditize.json`)||e.name.endsWith(`wpd.json`)||e.name.endsWith(`.json`)&&!e.name.includes(`manifest.json`)&&!e.name.includes(`info.json`))i=new TextDecoder().decode(e.data);else if(e.name.endsWith(`.png`)||e.name.endsWith(`.jpg`)||e.name.endsWith(`.jpeg`)||e.name.endsWith(`.webp`)){let t=e.name.endsWith(`.jpg`)||e.name.endsWith(`.jpeg`)?`image/jpeg`:`image/png`,n=new Blob([e.data],{type:t});a=URL.createObjectURL(n)}if(i){let e=JSON.parse(i),t=n(e,a);t?this.onProjectLoad&&this.onProjectLoad(t):alert(`解包成功，但未在 JSON 中找到合法的 columns 或 calibration 字段。`)}else alert(`未在 .tar 归档中找到项目数据 JSON 文件。`)}catch(e){alert(`解析 .tar 归档失败：`+e.message)}},t.readAsArrayBuffer(e);return}let r=new FileReader;r.onload=e=>{try{let t=e.target?.result||``,r=JSON.parse(t),i=n(r,null);i?this.onProjectLoad&&this.onProjectLoad(i):alert(`项目文件格式不匹配：未找到有效的 columns 或 calibration 配置。`)}catch{alert(`读取项目文件失败：无效的 JSON 格式。`)}},r.readAsText(e,`utf-8`)}openRpcConfigModal(e){let t=document.createElement(`div`);t.className=`modal-backdrop`;let n=this.rpcClient.getStatus();t.innerHTML=`
  ...
  `,v.appendChild(P),N.addEventListener(`click`,()=>{let e=P.style.display===`none`;P.style.display=e?`block`:`none`}),P.querySelector(`#help-panel-close`)?.addEventListener(`click`,()=>{P.style.display=`none`});let F=document.createElement(`div`);F.className=`canvas-hud`,F.innerHTML=`
    <span class="hud-dot"></span>
    <span id="hud-text">就绪：单击左键添加锚点拉伸轮廓 | 拖拽微调 | 右键删点 | 按 B 键即时透视二值化墨迹</span>
  `,v.appendChild(F);let I=null;function L(e,t=3e3){let n=document.getElementById(`hud-text`);n&&(n.textContent=e,I&&clearTimeout(I),I=window.setTimeout(()=>{n.textContent=`就绪：单击左键添加锚点拉伸轮廓 | 拖拽微调 | 右键删点 | 按 B 键即时透视二值化墨迹`,I=null},t))}let R=document.createElement(`footer`);R.className=`app-footer`,R.innerHTML=`
  ...
  `;let z=new f(v,i,a,{onTaxaChange:e=>{u?.updateData(z.data),d?.updateData(z.data),Y()},onDataChange:()=>{u?.updateData(z.data),d?.updateData(z.data),m?.updateHistoryState(),Y(),l()},onHoverInfo:e=>{let t=document.getElementById(`footer-cursor`),n=document.getElementById(`footer-depth`),r=document.getElementById(`footer-pollen`);if(e){if(t&&(t.innerHTML=`坐标: <code>X:${e.worldX} Y:${e.worldY}</code>`),n){let t=z.data.calibration.unit;n.innerHTML=e.horizonDepth!==void 0&&e.horizonDepth!==null?`深度: <code>${e.depth===void 0?`--`:e.depth+` `+t}</code> <strong style="color: #38bdf8; margin-left: 6px;">[层位: ${e.horizonDepth} ${t}]</strong>`:`深度: <code>${e.depth===void 0?`--`:e.depth+` `+t}</code>`}r&&(r.innerHTML=`丰度: <code>${e.percent===void 0?`--`:e.percent+`%`}</code>`)}},onFilterChange:(e,t)=>{m?.updateFilterState(e,t)},onDropFile:e=>{K(e)},onStatusNotice:e=>{L(e,2500)},onOpenCalibration:()=>{G.openCalibrationModal()},onOpenFileDialog:()=>{document.getElementById(`file-input-image`)?.click()}}),B=z.data.columns.length>0?3:1;z.setWorkflowStage(B);let V=document.createElement(`div`);V.className=`workflow-action-bar`,V.style.cssText=`
    position: absolute;
    bottom: 18px;
    left: 50%;
    ...
    pointer-events: auto;
  `,v.appendChild(V);function H(){let e=p[B];z.setWorkflowStage(B),m!==void 0&&m&&m.setWorkflowStep(B),d!==void 0&&d&&d.setWorkflowStage(B),V.innerHTML=`
      <div style="display: flex; align-items: center; gap: 8px; min-width: 0;">
        <span style="background: ${s.color.column.activeBadge}; color: #fff; font-weight: 700; font-size: 10px; padding: 2px 7px; border-radius: 9999px; flex-shrink: 0;">S${B}</span>
      ...
      <div style="display: flex; align-items: center; gap: 6px; flex-shrink: 0;">
        ${B>1?`<button id="btn-wf-prev" class="tool-btn" style="padding: 3px 8px; font-size: 10px;">↺ 上一步</button>`:``}
    ...
    `,V.querySelector(`#btn-wf-prev`)?.addEventListener(`click`,()=>{B>1&&(--B,H())}),V.querySelector(`#btn-wf-next`)?.addEventListener(`click`,async()=>{if(B===0)document.getElementById(`file-input-image`)?.click();else if(B===1)B=2,H(),L(`✅ 数据有效区 (ROI) 已锁定！可在此阶段开启图像去横线并按 B 键预览，满意后进入分列。`,4500);else if(B===2){L(`正在基于纯数据有效区推导各花粉属种垂直基线...`,5e3);let e=z.data.calibration,n=await t.detectColumnsInRoi({x0:e.dataXMin,x1:e.dataXMax,y0:e.dataYMin,y1:e.dataYMax});B=3,u?.updateData(z.data),d?.updateData(z.data),H(),Y(),L(`✅ 成功切分 ${n.length} 个属种列！请在侧边栏核对名单或使用 ▲/▼ 对调顺位。`,4e3)}else if(B===3)B=4,H(),L(`👉 请在右侧属性检查器核查或微调两点式深度标尺与各列物理刻度齿。`,4e3);else if(B===4){L(`正在提取各列花粉多边形轮廓与显著控制手柄...`,8e3);let e=z.data.columns;for(let n of e){let e=await t.digitizeColumn(n.id);e.length>0&&(n.controlPoints=e)}B=5,u?.updateData(z.data),d?.updateData(z.data),H(),Y(),L(`✅ 数字化完成！绿色半透明逆向对比层已开启，可直接在画布拖拽控制点微调。`,4500)}else B===5?(B=6,H(),G.openExportModal(),L(`🔍 已进入地学校验阶段：正在核验 100% 丰度总和自检门禁。`,4e3)):B===6?(B=7,H(),G.openExportModal()):B===7&&G.openExportModal()})}u=new _(z.data,{onSelectTaxa:e=>{z.setActiveTaxa(e),Y()},onToggleVisible:e=>{let t=z.data.columns.find(t=>t.id===e);t&&(t.visible=!t.visible,z.requestRender(),u?.updateData(z.data))},onChangeCurveType:e=>{let t=z.data.columns.find(t=>t.id===e);t&&(t.curveType=`linear`,a.push(`Ensure Curve Linear`,z.data.columns,z.data.activeTaxaId),z.requestRender(),u?.updateData(z.data))},onUpdateTaxaColor:(e,t)=>{let n=z.data.columns.find(t=>t.id===e);n&&(n.color=t,z.requestRender(),u?.updateData(z.data))},onBatchImportTaxa:e=>{z.batchUpdateTaxa(e),u?.updateData(z.data),m?.updateHistoryState(),Y(),l(),L(`✅ 成功批量导入 ${e.length} 个属种名单并完成自动拓展对齐！`,3500)},onSwapTaxaNames:(e,t)=>{let n=z.data.columns;if(e>=0&&e<n.length&&t>=0&&t<n.length){let r=n[e].name;n[e].name=n[t].name,n[t].name=r,a.push(`Swap Taxa Names (${n[e].name} <-> ${n[t].name})`,n,z.data.activeTaxaId),u?.updateData(z.data),d?.updateData(z.data),m?.updateHistoryState(),Y(),l(),L(`🔀 已对调属种顺位: ${n[e].name} 与 ${n[t].name}`)}},onInsertGapColumn:e=>{let t=z.data.columns,n=t.findIndex(t=>t.id===e),r=n===-1?t.length:n+1,i=n===-1?t[t.length-1]:t[n],o=i?i.endX:z.data.calibration.dataXMin,s=i?i.endX-i.startX:60,c={id:`col_${Date.now()}_gap`,name:`Gap_Col_${r+1}`,color:`#94a3b8`,startX:o,endX:o+s,maxPercent:20,tickEndX:o+s,unit:`%`,isLocked:!1,curveType:`linear`,visible:!0,controlPoints:[],scale_type:`linear`,startValue:0,tickValue:20,plotType:`area`};t.splice(r,0,c),z.data.activeTaxaId=c.id,a.push(`Insert Gap Column at ${r+1}`,t,c.id),z.requestRender(),u?.updateData(z.data),d?.updateData(z.data),m?.updateHistoryState(),Y(),l(),L(`➕ 已插入空缺占位列 [Gap_Col_${r+1}]，后续属种名字已顺延后推！`,4e3)}});let U=new S(document.body,t,e=>{L(`✅ 论文元数据已保存更新！已同步至 XLSX / LiPD 导出引擎。`,3500)}),W=new x(document.body,z.data,t,e=>{L(`✅ 成功关联年代模型 [${e.metadata.curve_type||`Median`}]！导出时将自动注入日历年代与 95% 置信区间。`,4e3)}),G=new y(document.body,z.data,t,e=>{z.data.calibration=e,a.push(`Update Calibration`,z.data.columns,z.data.activeTaxaId),z.requestRender(),d?.updateData(z.data),Y()},e=>{z.loadNewDiagram(e),a.reset(e.columns,e.activeTaxaId),B=3,H(),u?.updateData(z.data),d?.updateData(z.data),m?.updateHistoryState(),m?.updateScale(z.viewport.scale),Y(),L(`✅ 成功载入 Straditize 科学项目包 (.tar)！已 100% 还原全部属种、刻度钉与控制点。`,4500)});d=new b(z.data,a,{onDataChange:()=>{z.requestRender(),u?.updateData(z.data),m?.updateHistoryState(),Y()},onSelectTaxa:e=>{z.setActiveTaxa(e),u?.updateData(z.data),Y()},onToggleCollapse:e=>{A(e)},onDigitizeActiveColumn:async()=>{let e=z.getActiveColumn();if(!e)return;let n=await t.digitizeColumn(e.id);n.length>0&&(e.controlPoints=n,a.push(`Re-digitize ${e.name}`,z.data.columns,z.data.activeTaxaId),z.requestRender(),u?.updateData(z.data),d?.updateData(z.data),L(`⚡ 属种 ${e.name} 轮廓已根据图像算法完成重识别！`))},onOpenDataViewer:()=>{G.openExportModal()},onToggleLayerVisibility:(e,t)=>{e===`ghost`&&(z.viewport.showGhosting=t,z.requestRender(),L(t?`🟢 绿色原位半透明对比层已开启`:`绿色对比层已关闭`))},onChangeDegridStrength:e=>{z.setDegridStrength(e),L(`去网格横线灵敏度设为: ${e.toUpperCase()} (按 B 键透视查看红色切除预览)`)}});async function K(e){let n=e.name.toLowerCase();if(n.endsWith(`.tar`)||n.endsWith(`.json`)||n.endsWith(`.tar.gz`)){G.openProjectFile(e);return}if(!e.type.startsWith(`image/`)){L(`请选择或拖入有效的图片文件或项目文件 (.tar / .json)`);return}if(a.canUndo()&&!window.confirm(`当前项目有未保存修改，重新加载图片将清空当前工作区。是否继续？`))return;L(`正在载入地质图谱: ${e.name}...`,8e3);let r=new FileReader;r.onload=async n=>{let r=n.target?.result;if(!r)return;let i=new Image;i.onload=async()=>{let n=i.naturalWidth,o=i.naturalHeight,s=8e3,c=12e3;if(n>s||o>c){if(!window.confirm(`【图像尺寸过大提示】\n当前图像尺寸为 ${n}×${o} px，超过建议最大限制 (${s}×${c} px)。\n直接加载可能会耗尽浏览器内存导致崩溃。\n\n点击【确定】以 50% 降采样安全加载 (${Math.round(n/2)}×${Math.round(o/2)} px)；\n点击【取消】中止加载。`)){L(`已取消加载超限大图`);return}let e=q(i,.5);r=e.dataUrl,n=e.w,o=e.h}else n>=6e3&&o>=9e3&&L(`提示: 图像尺寸较大 (${n}×${o} px)，建议在充足内存环境下操作。`,4e3);let l=await t.loadCustomImage(r,n,o,e.name);z.loadNewDiagram(l),a.reset([],``),B=1,H(),u?.updateData(z.data),m?.updateHistoryState(),m?.updateScale(z.viewport.scale),m?.updateFilterState(z.viewport.imageMode,z.viewport.showBinaryOverlay),Y(),L(`✅ 成功载入图谱 [${e.name}] (${n}×${o})！请在画布上调整数据有效区 (Step 1)，随后点击下方推进。`,5e3),t.detectDeskew().then(e=>{if(e&&e.has_skew&&Math.abs(e.suggested_rotation_angle)>=.3){let n=e.suggested_rotation_angle,r=document.createElement(`div`);r.className=`deskew-notice-banner`,r.style.cssText=`position: fixed; top: 52px; right: 20px; z-index: 9999;`,r.innerHTML=`
            ...
            `,document.body.appendChild(r),r.querySelector(`#btn-deskew-apply`)?.addEventListener(`click`,async()=>{r.remove(),L(`正在旋转矫正图谱 (${n}°)...`,5e3);let e=await t.rotateImage(n);if(e&&e.success){let e=await t.getDiagramData();z.loadNewDiagram(e),a.reset([],``),B=1,H(),L(`✅ 已水平矫正图谱！有效区已重置。`,3500)}}),r.querySelector(`#btn-deskew-ignore`)?.addEventListener(`click`,()=>{r.remove()})}})},i.src=r},r.readAsDataURL(e)}function q(e,t=.5){let n=Math.round(e.naturalWidth*t),r=Math.round(e.naturalHeight*t),i=document.createElement(`canvas`);i.width=n,i.height=r;let a=i.getContext(`2d`);return a&&(a.imageSmoothingEnabled=!0,a.imageSmoothingQuality=`high`,a.drawImage(e,0,0,n,r)),{dataUrl:i.toDataURL(`image/png`),w:n,h:r}}async function J(e){L(`正在切换内置范例图谱: ${e}...`,5e3);let n=await t.loadSampleDiagram(e);z.loadNewDiagram(n),a.reset(n.columns,n.activeTaxaId),B=3,H(),u?.updateData(z.data),m?.updateHistoryState(),m?.updateScale(z.viewport.scale),m?.updateFilterState(z.viewport.imageMode,z.viewport.showBinaryOverlay),Y(),L(`✅ 已载入范例: ${{hoya:`Hoya del Castillo 花粉剖面`,verification:`标定验证地质图谱`,beginner:`初学者沉积图谱`}[e]||e}，已自动居中重置！`,3500)}m=new h(a,t.getStatus(),{onFit:()=>{z.fitToScreen(),m?.updateScale(z.viewport.scale)},onReset100:()=>{z.resetZoom100(),m?.updateScale(z.viewport.scale)},onZoomIn:()=>{let e=z.canvas.getBoundingClientRect();z.viewport.zoomAt({x:e.width/2,y:e.height/2},1.25),z.requestRender(),m?.updateScale(z.viewport.scale)},onZoomOut:()=>{let e=z.canvas.getBoundingClientRect();z.viewport.zoomAt({x:e.width/2,y:e.height/2},.8),z.requestRender(),m?.updateScale(z.viewport.scale)},onUndo:()=>{let e=a.undo();e&&(z.data.columns=e.columns,z.data.activeTaxaId=e.activeTaxaId,z.requestRender(),u?.updateData(z.data),m?.updateHistoryState())},onRedo:()=>{let e=a.redo();e&&(z.data.columns=e.columns,z.data.activeTaxaId=e.activeTaxaId,z.requestRender(),u?.updateData(z.data),m?.updateHistoryState())},onDigitize:async()=>{let e=z.getActiveColumn();if(!e)return;let n=await t.digitizeColumn(e.id);n.length>0&&(e.controlPoints=n,a.push(`Re-digitize ${e.name}`,z.data.columns,z.data.activeTaxaId),z.requestRender(),u?.updateData(z.data))},onExport:async e=>{let n=await t.exportData(e);G.openExportModal(n,e)},onSaveProject:()=>{G.saveProjectFile(),L(`💾 数字化项目已打包导出为标准归档包 (.tar)！`,3500)},onOpenProjectFile:e=>{G.openProjectFile(e)},onOpenCalibrationModal:()=>{G.openCalibrationModal()},onOpenMetadataModal:()=>{U.open()},onOpenAgeDepthModal:()=>{W.open()},onToggleRpcConfig:()=>{G.openRpcConfigModal(()=>{m.updateStatus(t.getStatus())})},onOpenFile:e=>{K(e)},onLoadSample:e=>{J(e)},onChangeImageMode:e=>{z.viewport.imageMode=e,z.requestRender(),m?.updateFilterState(e,z.viewport.showBinaryOverlay),L(`底图滤镜模式切换为: ${e}`)},onToggleBinaryOverlay:()=>{let e=z.viewport.toggleBinaryOverlay();z.requestRender(),m?.updateFilterState(z.viewport.imageMode,e),L(e?`透视遮罩: 墨迹高亮模式 [开启] (青蓝=保留花粉，红=切除横线，快捷键 B)`:`透视遮罩: [关闭]`)},onChangeDegridStrength:e=>{z.setDegridStrength(e),L(`去网格横线灵敏度设为: ${e.toUpperCase()} (按 B 键透视查看红色切除预览)`)},onSelectToolMode:e=>{z.setToolMode(e),m.setToolMode(e);let t=z.toolModeManager.getToolDescription(e),n=document.getElementById(`footer-tool-mode`);n&&(n.innerHTML=`模式: <strong>${t.name} (${t.shortcut})</strong>`),L(`工具模式切换: ${t.name} (${t.shortcut}) - ${t.hint}`)},onToggleSidebar:()=>{j()},onToggleInspector:()=>{M()},onStepClick:e=>{B=e,H();let t=p[B];L(`切换至步骤 ${e}: ${t.stepName} - ${t.title}`)}}),(t.getStatus().isDesktopMode??i.isDesktopMode)&&m.setDesktopMode(!0),g.appendChild(u.getElement()),g.appendChild(v),g.appendChild(d.getElement()),e.appendChild(m.getElement()),e.appendChild(g),e.appendChild(R),D&&k(!0),O&&A(!0),H(),window.addEventListener(`keydown`,e=>{let t=e.target;t?.tagName!==`INPUT`&&t?.tagName!==`TEXTAREA`&&(e.ctrlKey&&e.code===`KeyB`||e.key===`[`&&!e.ctrlKey&&!e.metaKey?(e.preventDefault(),j()):(e.ctrlKey&&e.shiftKey&&e.code===`KeyI`||e.key===`]`&&!e.ctrlKey&&!e.metaKey)&&(e.preventDefault(),M()))});function Y(){let e=z.getActiveColumn(),t=document.getElementById(`footer-active-taxa`),n=document.getElementById(`footer-anchors`),r=document.getElementById(`footer-dimensions`),i=document.getElementById(`footer-zoom`);r&&(r.innerHTML=`图像: <code>${z.data.imageWidth}×${z.data.imageHeight}</code>`),i&&(i.innerHTML=`缩放: <code>${Math.round(z.viewport.scale*100)}%</code>`),e&&t&&(t.innerHTML=`当前属种: <span style="color: ${e.color};">●</span> <strong>${e.name}</strong>`),e&&n&&(n.innerHTML=`锚点数: <code>${e.controlPoints.filter(e=>e.isManual).length} 手动 / ${e.controlPoints.length} 总计</code>`)}z.canvas.addEventListener(`wheel`,()=>{m?.updateScale(z.viewport.scale)}),window.addEventListener(`resize`,()=>{z.handleResize()}),requestAnimationFrame(()=>{z.handleResize(),z.fitToScreen(),m?.updateScale(z.viewport.scale)}),Y(),m?.updateScale(z.viewport.scale),m?.updateFilterState(z.viewport.imageMode,z.viewport.showBinaryOverlay);try{let e=localStorage.getItem(o);if(e){let t=JSON.parse(e);if(t&&t.data&&Array.isArray(t.data.columns)&&t.data.columns.length>0&&Date.now()-t.timestamp<6048e5){let e=new Date(t.timestamp).toLocaleTimeString(),n=document.createElement(`div`);n.className=`draft-recovery-banner`,n.style.cssText=`position: fixed; top: 48px; left: 50%; transform: translateX(-50%); z-index: 9999;`,n.innerHTML=`
        ...
        `,document.body.appendChild(n),n.querySelector(`#btn-restore-draft`)?.addEventListener(`click`,()=>{z.loadNewDiagram(t.data),a.reset(t.data.columns,t.data.activeTaxaId),u?.updateData(z.data),d?.updateData(z.data),m?.updateHistoryState(),Y(),n.remove(),L(`✅ 成功恢复上次自动暂存的项目草稿！`,3500)}),n.querySelector(`#btn-discard-draft`)?.addEventListener(`click`,()=>{localStorage.removeItem(o),n.remove()})}}}catch{}console.log(`Straditize Modern Frontend Initialized Successfully`)}window.addEventListener(`DOMContentLoaded`,()=>{C().catch(e=>{console.error(`Failed to bootstrap Straditize frontend:`,e)})});
//# sourceMappingURL=index-DAGOHVkv.js.map