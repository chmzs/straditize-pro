...
def create_matplotlib_figure(headless=None):
    ...
    if headless:
        ...
        # Matplotlib 3.10 deprecated the public setter; keep the private
        # identifier for compatibility with the headless close bookkeeping.
        fig._number = next(_headless_figure_numbers)
        return fig
...
class Straditizer(LabelSelection):
    ...
    def final_df(self):
        if (self.data_reader is None or self.data_reader.full_df is None or
            ...
            return None
        return self._finalize_df(self.data_reader.sample_locs.copy(True))
    ...
    def _finalize_df(self, df, strict=False):
        """Combine column information and optionally require calibration.

        ``strict=True`` is intended for scientific-data export: missing or
        invalid axis calibration must not silently leave pixel coordinates in
        the output.
        """
        try:
            ...
            if strict:
                raise ValueError(
                    'Y-axis calibration is required before export.') from exc
        try:
            ...
            if strict:
                raise ValueError(
                    'X-axis calibration is required before export.') from exc
    ...
    def close(self):
        import matplotlib.pyplot as plt
        if not hasattr(self, 'image') and not hasattr(self, 'ax'):
            return
