...
class DataReader(LabelSelection):
    ...
    def __init__(self, image, ax=None, extent=None,
                 plot=True, children=None, parent=None, magni=None,
    ...
    def new_child_for_cols(self, columns, cls, plot=True):
        ...
        try:
            self.image.putalpha(Image.fromarray(self_alpha))
    ...
    def create_exaggerations_reader(self, factor, cls=None,
        ...
        try:
            ...
            ret.image = Image.fromarray(np.zeros_like(np.asarray(self.image)))
        # update plot and binary image
        ret.hline_locs = self.hline_locs
        ret.vline_locs = self.vline_locs
        return ret
    ...
    def recognize_xaxes(self, fraction=0.3, min_lw=1, max_lw=None,
        ...
        if mask.any():
            ...
            for i, row in enumerate(rows[1:], 1):
                previous = row_sums[rows[i - 1]]
                current = row_sums[row]
                changed = (current != 0 if previous == 0 else
                           np.abs(current / previous - 1) > 0.05)
                if changed:
                    rows = rows[:i]
                    break
    ...
    def plot_potential_samples(
            self, excluded=False, ax=None, plot_kws=None, *args, **kwargs):
