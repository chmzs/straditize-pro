...
def main(exec_=True):
    # Instant console hiding on Windows desktop launch (double-click / no args)
    ...
    # Modern Straditize v2.0 (straditize pro) routing:
    # 1. Double click / no args: Desktop Mode
    # 2. straditize serve [--port N]: Server Mode
    if len(sys.argv) == 1 or (len(sys.argv) > 1 and sys.argv[1] == 'serve'):
        from straditize_core.rpc_server import main as rpc_main
        return rpc_main()

    # 3. Headless batch CLI commands (extract, run-project)
    if len(sys.argv) > 1 and sys.argv[1] in ('extract', 'run-project'):
        from straditize_core.cli import main as cli_main
        return cli_main()
