...
class StraditizerTest(unittest.TestCase):
    ...
    def test_close_without_plot_is_idempotent(self):
        stradi = Straditizer(osp.join(test_dir, 'test_figures',
                                      'basic_diagram.png'), plot=False)
        stradi.close()
        stradi.close()

    def test_finalize_df_strict_requires_y_axis_calibration(self):
        stradi = Straditizer(osp.join(test_dir, 'test_figures',
                                      'basic_diagram.png'), plot=False)
        with self.assertRaisesRegex(ValueError, 'Y-axis'):
            stradi._finalize_df(pd.DataFrame({0: [1.0]}), strict=True)
        stradi.close()

    def test_final_df_preserves_missing_values(self):
        stradi = Straditizer(osp.join(test_dir, 'test_figures',
                                      'basic_diagram.png'), plot=False)
        values = pd.DataFrame({0: [1.0, np.nan]})
        reader = mock.Mock(full_df=values, sample_locs=values.copy())
        stradi.data_reader = reader
        stradi._finalize_df = lambda df: df
        result = stradi.final_df
