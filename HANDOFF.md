...
## 2026-09-18 — 落地 Vector-First 连续矢量切片、拐点共振层位与子 ROI 深度对齐强锁架构

### 核心地学算法与架构交付
1. **Vector-First 连续多边形解析切片引擎 (`straditize_core/digitize.py`)**：
   - 彻底解耦“图像物理分辨率 (440px)”与“高密度测样点序列”的矛盾，以连续封闭矢量多边形为核心载体；
   - 实现 `extract_vector_polygon` 与 `slice_vector_polygon_at_depths`，支持在任意浮点亚像素深度进行水平射线解析求交（Ray-Casting Intersection）；
   - 新增单元测试 `tests/test_vector_first_digitize.py` 严格验证三角与非线性峰谷的多层位求交准确性（1/1 PASSED）。

2. **多属种物理拐点共振层位发现 (`extract_horizon_consensus`)**：
   - 遵照地学非均匀采样原则，彻底废黜“默认强行 2cm 等间距伪插值”的主导地位；
   - 算法横向聚类所有属种在相同高度上共同发生的曲率极值与折角，逆向还原当年钻孔采样的非均匀真实层位；
   - 新增测试 `tests/test_horizon_consensus.py` 验证多属种拐点横向聚类（1/1 PASSED）。

3. **子 ROI 垂直深度对齐强锁 (Y-Axis Lock)**：
   - 用户在 S1 阶段点击 `[➕ 追加子有效区]` 时，新分区自动 100% 继承并锁死主图的顶界深度、底界深度与 Y 像素跨度；
   - 用户仅需在画布上横向框选 X 范围，确保多指标表格在时间轴与深度轴上具备严格的物理共时性；
   - 在真实 MS Edge 浏览器中通过 Playwright 验证了子 ROI 深度强锁与创建反馈（见实机截图 `ui_sub_roi_added.png`）。

4. **测试与持续集成**：
   - 全套自动化测试用例扩充至 **83/83 100% Passed (17.03s)**；
   - 前端 9 项自测全部绿灯通过；
   - 代码检查 0 错误 0 警告，工作树干净。

---

## 2026-09-18 — 落地方式2增量升级架构：ComponentManager 扩展包管理器与零重启热插拔就地激活

### 核心架构与扩展性交付 (轻量版增量扩展体系落地)
1. **轻量版组件管理器 (`straditize_core/components/manager.py`)**：
   - 遵照用户无管理员权限原则，标准安装至 `%APPDATA%/Straditize/components/`（Linux/macOS 映射到用户数据目录）；
   - 严密内置防 Zip-Slip 路径逃逸安全提取与 SHA256 完整性哈希校验；
   - 具备异步后台多镜像分流下载（GitHub Releases + 国内镜像加速代理），带实时流式进度反馈；
   - 完整支持离线环境学者“U 盘拷贝 `age-modeling.zip` 手动导入安装”逃生通道。

2. **前端免重启就地激活 (Hot-Plug)**：
   - 在 `AgeDepthModal.ts` 的【测年数据与建模向导】中嵌入增量组件状态卡片；
   - 初始状态显示 `未安装 (需增量包)`，点击一键下载或离线导入后，自动轮询进度条；
   - 安装完成后**无需杀掉程序重启**，状态瞬间切换为 `✓ 已安装 (1.0.0)`，按钮即刻解锁激活。

3. **测试与持续集成**：
   - 新增 `tests/test_component_manager.py` 覆盖 4 项安全、解压、安装元数据与卸载测试（4/4 全部通过）；
   - 全量自动化测试用例扩充至 **81/81 100% Passed (20.20s)**，`pixi run lint` 0 错误 0 警告；
